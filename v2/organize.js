...
function effectiveBlueprint(){const migratedParents=Object.assign({},placements.parents||{});for(const [id,parent] of Object.entries(placements.parents||{})){migratedParents[id]=parent;const canonId=canonicalSection(id),canonParent=canonicalSection(parent);if(canonId!==canonParent)migratedParents[canonId]=canonParent;}const rows=allBlueprint().map(n=>({...n,parentId:migratedParents[n.id]??n.parentId}));const hidden=new Set(Object.keys(placements.deleted||{}).filter(k=>k.startsWith('node:')&&placements.deleted[k]).map(k=>canonicalSection(k.slice(5))));let changed=true;while(changed){changed=false;for(const n of rows)if(hidden.has(n.parentId)&&!hidden.has(n.id)){hidden.add(n.id);changed=true}}return rows.filter(n=>!hidden.has(n.id));}
