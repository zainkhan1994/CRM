...
function render(){document.body.classList.toggle('inbox-active',view==='inbox');document.body.classList.toggle('history-active',view==='history');$('#search').placeholder=view==='history'?'Search people, dates, companies, notes…':'Search people, companies, email…';$('#search').setAttribute('aria-label',view==='history'?'Search people, dates, companies, notes':'Search people, companies or email');$('.eyebrow').textContent=view==='inbox'?'Workspace / communications':'Workspace / personal CRM';document.querySelectorAll('[data-go]').forEach(b=>{if(b.closest('nav')){b.classList.toggle('active',b.dataset.go===view);b.setAttribute('aria-current',b.dataset.go===view?'page':'false')}});$('#title').textContent=names[view];document.querySelectorAll('[data-layout]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.layout===layout)));$('#view-options').hidden=['network','history'].includes(view);if(view==='inbox'){$('#content').innerHTML='<iframe sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox" class="inbox-frame" title="Inbox — Gmail snapshot" src="inbox.html?v=groups1"></iframe>';$('#pagination').innerHTML='';$('#result-count').textContent='';$('#view-options').hidden=true;return;}let list=filtered(),slice=list.slice(page*size,(page+1)*size);$('#result-count').textContent=`${list.length.toLocaleString()} ${view==='companies'?'companies':'contacts'}`;
